export function queryCoingecko(endpoint: string, params?: Record<string, string>): Promise<Response> {
    ...
    return fetch(`${url}/${endpoint}?x_cg_demo_api_key=${key}${paramString ? '&' + paramString : ''}`);
}
